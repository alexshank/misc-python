...
# TODO add dummy example data for blog post
# TODO this can be generated by a GPT model using actual data as example
# TODO (not concern about privacy of data, just need to show how it works)
...
INFIELDING_POSITIONS = set([
    # 1, # "P",   # Pitcher, handled separately
    3, # "1B",  # First Base
    4, # "2B",  # Second Base
    5, # "3B",  # Third Base
    6, # "SS",  # Shortstop
...
def read_in_roster() -> list[Player]:
    roster_file = os.path.join(os.path.dirname(__file__), "./inputs/05-19-2025-roster.csv")
    players = []
    ...
    with open(roster_file, mode='r') as file:
        ...
        for player in players:
            player.batting_skill = 100 * (1 + (len(players) - player.batting_skill))  # Reverse the skill level for optimization
...
def create_multiple_fielding_positions(players, num_configurations=9):
    ...
    for config_index in range(num_configurations):
        ...
        filtered_players = [player for player in players if player.attendance_0519]
...
def create_batting_order(players) -> list[Player]:
    ...
    batting_problem += pulp.lpSum(skill_levels[i] * position_weights[j] * x[i][j] for i in range(num_batters) for j in range(num_batters))
